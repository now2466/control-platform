...
`RosbridgeAdapter`는 robot별 websocket, 재연결(1/2/4/8초), odom/battery/control-status/path/compressed-camera 구독 및 JPEG cache를 구현했다. `Odometry`는 `map` 좌표로 간주하지 않는다. map-to-odom TF가 실제로 확인될 때까지 `tf_valid=false`, `MAP_TF_UNVERIFIED`로 표시한다.
...
따라서 기본 ROS 설정은 새 control/follow endpoint를 이름만 보존하고 `control_available=false`, `follow_available=false`로 둔다. 명령은 `UNSUPPORTED`이며 adapter는 `/cmd_vel`에 직접 쓰지 않는다. 실제 endpoint와 계약이 제공된 뒤에만 해당 플래그를 활성화하고 무이동 상태에서 상태·영상·stop acknowledgement부터 현장 검증해야 한다.
