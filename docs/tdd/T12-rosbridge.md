...
이후 `reset_stop`·`set_mode(MANUAL)` 응답, 수동 입력 중 `/cmd_vel`, 입력 중단 0.35초 이내 정지, `navigate` capability와 `/navigate_to_pose` action server를 무이동 상태에서 확인한 뒤에만 바퀴를 지면에 둔 저속 시험으로 진행한다. 로봇을 들어 옮긴 뒤에는 정지 확인 → 새 시작점 지정 → `localization-reset`/AMCL → 명시적 정지 해제 → 새 도착점 요청 순서를 지킨다. 정적 지도는 초기화하지 않는다.
