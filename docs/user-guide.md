...
- `위치 재설정(AMCL)`은 로봇을 손으로 들어 다른 위치에 둔 뒤 새 시작점을 AMCL에 적용하는 버튼이다. 정적 `map_260905` 파일은 지우거나 다시 그리지 않는다.
...
관리자용 `초기 위치 적용`은 기존 설정 API로, 정지·편대 해제·유효한 지도 TF 조건을 확인한다. 현장 시험에서 로봇을 들어 옮긴 뒤 쓰는 `위치 재설정(AMCL)`은 별도 operator API이며 정지 상태에서 `/initialpose`만 발행한다. 두 동작 모두 자동 주행을 시작하지 않는다.
