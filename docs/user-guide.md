...
- `위치 재설정(AMCL)`은 로봇을 손으로 들어 다른 위치에 둔 뒤 새 시작점을 AMCL에 적용하는 버튼이다. 기존 위치나 map TF가 지연된 상태에서도 복구에 사용할 수 있으며, 로봇은 연결된 정지 상태이고 속도가 0이어야 한다. 정적 `map_260905` 파일은 지우거나 다시 그리지 않는다.
