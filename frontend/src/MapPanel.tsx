...
type Robot = { robot_id: string; name: string; role: 'MASTER' | 'SLAVE'; pose?: { x: number; y: number; yaw: number; frame_id?: string } | null; pose_freshness?: string; trail?: Point[]; path?: Point[]; goal?: Goal | null; tf_valid?: boolean; tf_reason_code?: string | null; received_at?: string | null }
...
export default function MapPanel({ robots, selected, mapId, onSelect, onGoalChange, initialPose, onInitialPoseChange, onResetSelections, onNavigate, navigationReady = false, navigationBusy = false, navigationBlockers = [], onResetLocalization, localizationReady = false, localizationBusy = false, localizationBlockers = [], navigationStatus = '' }: { robots: Robot[]; selected: string; mapId?: string | null; onSelect: (id: string) => void; onGoalChange?: (goal: Goal | null) => void; initialPose?: Goal | null; onInitialPoseChange?: (pose: Goal | null) => void; onResetSelections?: () => void; onNavigate?: () => void; navigationReady?: boolean; navigationBusy?: boolean; navigationBlockers?: string[]; onResetLocalization?: () => void; localizationReady?: boolean; localizationBusy?: boolean; localizationBlockers?: string[]; navigationStatus?: string }) {
  ...
  const currentPoseAvailable = Boolean(active?.pose && active.pose_freshness === 'FRESH' && active.tf_valid)
  const setStartFromRobot = (robot: Robot) => {
    if (!robot.pose || robot.pose_freshness !== 'FRESH' || !robot.tf_valid) return
    onInitialPoseChange?.({ x: robot.pose.x, y: robot.pose.y, yaw: robot.pose.yaw, frame_id: robot.pose.frame_id ?? metadata?.frame_id ?? 'map' })
  ...
  return <section className="map-panel">
    <div className="map-toolbar"><h2>지도 · 로봇 위치</h2><div><button className={selectionMode === 'start' ? 'active' : ''} onClick={() => setSelectionMode('start')}>시작점 설정</button><button disabled={!currentPoseAvailable} title={!currentPoseAvailable ? '선택 로봇의 최신 map 위치가 필요합니다.' : undefined} onClick={() => active && setStartFromRobot(active)}>현재 위치를 시작점으로</button><button className={selectionMode === 'destination' ? 'active' : ''} onClick={() => setSelectionMode('destination')}>도착점 설정</button><button onClick={() => { setPreview(null); onGoalChange?.(null); onInitialPoseChange?.(null); onResetSelections?.() }}>설정 초기화</button><button disabled={!canResetLocalization} title={!canResetLocalization ? localizationBlockers.join(' ') : undefined} onClick={onResetLocalization}>{localizationBusy ? '위치 재설정 중…' : '위치 재설정(AMCL)'}</button><button disabled={!canNavigate} title={!canNavigate ? navigationReasons.join(' ') : undefined} onClick={onNavigate}>{navigationBusy ? '이동 요청 중…' : '시작점에서 도착점으로 이동'}</button><button onClick={() => setView({ scale: 1, offsetX: 0, offsetY: 0 })}>전체 보기</button><button onClick={() => zoom(1.25)}>확대</button><button onClick={() => zoom(.8)}>축소</button><button onClick={() => pan(-10, 0)}>←</button><button onClick={() => pan(10, 0)}>→</button><button className={following ? 'active' : ''} onClick={() => setFollowing(value => !value)}>선택 따라보기</button></div></div>
    ...
    <svg viewBox={`0 0 ${map.width} ${map.height}`} className="map">
      ...
      {robots.map(robot => robot.pose && <g key={robot.robot_id} onClick={event => { event.stopPropagation(); if (selectionMode === 'start' && selected === robot.robot_id) setStartFromRobot(robot); else onSelect(robot.robot_id) }} className={selected === robot.robot_id ? 'selected-robot' : ''}>{robot.trail?.length ? <polyline points={robot.trail.map(point => { const q = project(point); return `${q.x},${q.y}` }).join(' ')} fill="none" stroke="#60718a" strokeWidth="1" /> : null}{robot.path?.length ? <polyline points={robot.path.map(point => { const q = project(point); return `${q.x},${q.y}` }).join(' ')} fill="none" stroke="#f0b35b" strokeDasharray="2 2" strokeWidth="1" /> : null}{robot.goal ? <circle cx={project(robot.goal).x} cy={project(robot.goal).y} r="4" fill="none" stroke="#f0b35b" /> : null}{(() => { const point = project(robot.pose!); const stale = robot.pose_freshness !== 'FRESH' || !robot.tf_valid; return <><circle cx={point.x} cy={point.y} r="4" fill={stale ? '#89919f' : robot.role === 'MASTER' ? '#3182ce' : '#ed8936'} /><line x1={point.x} y1={point.y} x2={point.x + Math.cos(worldYawToCanvas(robot.pose!.yaw, map)) * 9} y2={point.y + Math.sin(worldYawToCanvas(robot.pose!.yaw, map)) * 9} stroke="white" strokeWidth="1.5" /><text x={point.x + 5} y={point.y - 5} fill="white" fontSize="5">{robot.name}{stale ? ` · 위치 지연 ${staleAgeLabel(robot.received_at)}` : ''}</text></> })()}</g>)}
