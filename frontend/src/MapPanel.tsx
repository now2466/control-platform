...
export default function MapPanel({ robots, selected, mapId, onSelect, onGoalChange }: { robots: Robot[]; selected: string; mapId?: string | null; onSelect: (id: string) => void; onGoalChange?: (goal: Goal | null) => void }) {
  ...
  const [layer, setLayer] = useState<'map' | 'scan' | 'local_costmap' | 'global_costmap'>('map')
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/robots/${selected}/sensor-layers`, { credentials: 'include' })
      ...
      .then(value => { if (alive) { setSensorLayers(value); setSensorError('') } })
  ...
  const costmap = layer === 'scan' || layer === 'map' ? undefined : sensorLayers?.costmaps.find(item => item.name === layer)
  const layerState = layer === 'map' ? 'OK' : layer === 'scan' ? sensorLayers?.scan.state : costmap?.state
  ...
  return <section className="map-panel"><div className="map-toolbar"><h2>지도 · 로봇 위치</h2><div><button onClick={() => setView({ scale: 1, offsetX: 0, offsetY: 0 })}>전체 보기</button><button onClick={() => zoom(1.25)}>확대</button><button onClick={() => zoom(.8)}>축소</button><button onClick={() => pan(-10, 0)}>←</button><button onClick={() => pan(10, 0)}>→</button><button className={following ? 'active' : ''} onClick={() => setFollowing(value => !value)}>선택 따라보기</button></div></div><div className="sensor-toolbar"><span>선택 로봇 센서</span>{(['map', 'scan', 'local_costmap', 'global_costmap'] as const).map(value => <button key={value} className={layer === value ? 'active' : ''} onClick={() => setLayer(value)}>{value === 'map' ? '지도' : value === 'scan' ? 'Scan' : value === 'local_costmap' ? 'Local costmap' : 'Global costmap'}</button>)}</div>{error && <div className="error">{error}</div>}<svg ref={svgRef} viewBox={`0 0 ${map.width} ${map.height}`} className="map" onPointerDown={onPointerDown} onPointerUp={onPointerUp}>{metadata ? <image href={imageHref} width={map.width} height={map.height} preserveAspectRatio="none" transform={`translate(${view.offsetX} ${view.offsetY}) scale(${view.scale})`} onLoad={loadOccupancy} /> : <rect width={map.width} height={map.height} fill="#18202b" />}{layer === 'scan' && active?.pose && sensorLayers?.scan.state === 'OK' && <g className="scan-layer">{sensorLayers.scan.rays.map((ray, index) => { const end = project({ x: active.pose!.x + Math.cos(active.pose!.yaw + ray.angle_rad) * ray.range_m, y: active.pose!.y + Math.sin(active.pose!.yaw + ray.angle_rad) * ray.range_m }); const startPoint = project(active.pose!); return <line key={index} x1={startPoint.x} y1={startPoint.y} x2={end.x} y2={end.y} stroke="#62d7d1" strokeWidth=".7" /> })}</g>}{costmap?.state === 'OK' && <g className="costmap-layer">{costmap.cells.filter(cell => cell.occupied).map((cell, index) => { const point = project(cell); return <rect key={index} x={point.x - 2} y={point.y - 2} width="4" height="4" fill={layer === 'local_costmap' ? '#f87171' : '#c084fc'} opacity=".7" /> })}</g>}{robots.map(robot => robot.pose && <g key={robot.robot_id} onClick={event => { event.stopPropagation(); onSelect(robot.robot_id) }} className={selected === robot.robot_id ? 'selected-robot' : ''}>{robot.trail?.length ? <polyline points={robot.trail.map(point => { const q = project(point); return `${q.x},${q.y}` }).join(' ')} fill="none" stroke="#60718a" strokeWidth="1" /> : null}{robot.path?.length ? <polyline points={robot.path.map(point => { const q = project(point); return `${q.x},${q.y}` }).join(' ')} fill="none" stroke="#f0b35b" strokeDasharray="2 2" strokeWidth="1" /> : null}{robot.goal ? <circle cx={project(robot.goal).x} cy={project(robot.goal).y} r="4" fill="none" stroke="#f0b35b" /> : null}{(() => { const point = project(robot.pose!); const stale = robot.pose_freshness !== 'FRESH' || !robot.tf_valid; return <><circle cx={point.x} cy={point.y} r="4" fill={stale ? '#89919f' : robot.role === 'MASTER' ? '#3182ce' : '#ed8936'} /><line x1={point.x} y1={point.y} x2={point.x + Math.cos(worldYawToCanvas(robot.pose!.yaw, map)) * 9} y2={point.y + Math.sin(worldYawToCanvas(robot.pose!.yaw, map)) * 9} stroke="white" strokeWidth="1.5" /><text x={point.x + 5} y={point.y - 5} fill="white" fontSize="5">{robot.name}{stale ? ` · 위치 지연 ${staleAgeLabel(robot.received_at)}` : ''}</text></> })()}</g>)}{preview && <g className="goal-preview"><circle cx={project(preview).x} cy={project(preview).y} r="4" fill="none" stroke="#fff" /><line x1={project(preview).x} y1={project(preview).y} x2={project(preview).x + Math.cos(worldYawToCanvas(preview.yaw, map)) * 12} y2={project(preview).y + Math.sin(worldYawToCanvas(preview.yaw, map)) * 12} stroke="#fff" /></g>}</svg>{cellError && <p className="map-warning">{cellError}</p>}{sensorError && <p className="map-warning">센서 상세: {sensorError}</p>}{layer !== 'map' && <p className="map-note">{layerLabel} · {layerState === 'UNSUPPORTED' ? '지원하지 않음' : layerState === 'STALE' ? '데이터 지연' : layerState === 'ERROR' ? '오류' : layerState === 'OK' ? 'OK' : '상태 확인 중'}</p>}{robots.some(robot => !robot.tf_valid) && <p className="map-warning">TF 변환을 확인할 수 없어 편대 거리·방위각을 표시하지 않습니다.</p>}<p className="map-note">{selected ? `선택: ${selected}` : '로봇을 선택하세요'} · 지도 빈 공간을 클릭하고 드래그하면 목표 미리보기를 표시합니다. 명령은 전송하지 않습니다.</p></section>
}
