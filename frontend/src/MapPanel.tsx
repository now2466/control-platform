...
export default function MapPanel({ robots, selected, mapId, onSelect, onGoalChange, initialPose, onInitialPoseChange, onResetSelections }: { robots: Robot[]; selected: string; mapId?: string | null; onSelect: (id: string) => void; onGoalChange?: (goal: Goal | null) => void; initialPose?: Goal | null; onInitialPoseChange?: (pose: Goal | null) => void; onResetSelections?: () => void }) {
  ...
  const onPointerDown = (event: PointerEvent<SVGSVGElement>) => { if (event.button === 0) start.current = toCanvas(event) }
  ...
  const onClick = (event: PointerEvent<SVGSVGElement>) => { selectPoint(event) }
  ...
  return <section className="map-panel">
    ...
    <svg viewBox={`0 0 ${map.width} ${map.height}`} className="map" onPointerDown={onPointerDown} onClick={onClick}>
      ...
      {preview && <g className="goal-preview"><circle cx={project(preview).x} cy={project(preview).y} r="5" fill="none" stroke="#fff" strokeWidth="1.5" /><line x1={project(preview).x} y1={project(preview).y} x2={project(preview).x + Math.cos(worldYawToCanvas(preview.yaw, map)) * 12} y2={project(preview).y + Math.sin(worldYawToCanvas(preview.yaw, map)) * 12} stroke="#fff" strokeWidth="2" /><text x={project(preview).x + 6} y={project(preview).y + 5} fill="white" fontSize="5">도착점</text></g>}
    </svg>
