...
test('selected robot scan and costmap overlays show map-projected points and unsupported state clearly', async () => {
  const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
    const url = String(input)
    if (url.includes('/sensor-layers')) return Promise.resolve(new Response(JSON.stringify(url.includes('robot_2') ? { robot_id: 'robot_2', scan: { state: 'UNSUPPORTED', points: [] }, costmaps: [{ name: 'local_costmap', state: 'UNSUPPORTED', cells: [] }, { name: 'global_costmap', state: 'UNSUPPORTED', cells: [] }] } : { robot_id: 'robot_1', scan: { state: 'OK', points: [{ x: 1.1, y: 1, range_m: .1 }, { x: 2, y: 1, range_m: 1 }], point_count: 2, min_range_m: .1 }, costmaps: [{ name: 'local_costmap', state: 'OK', cells: [{ x: 1.5, y: 1.5, occupied: true }] }, { name: 'global_costmap', state: 'OK', cells: [] }] }), { status: 200 }))
  ...
  expect(screen.getByText(/최근접 0.100 m/)).toBeTruthy()
  expect(document.querySelectorAll('.scan-layer circle').length).toBe(2)
  expect(document.querySelector('.scan-layer circle')?.getAttribute('fill')).toBe('#ff334f')
...
test('changing selection clears old sensor layers and ignores a mismatched response', async () => {
  ...
  expect(document.querySelectorAll('.scan-layer circle').length).toBe(1)
  rerender(<MapPanel robots={[robot, { ...robot, robot_id: 'robot_2', name: 'Pinky Slave', role: 'SLAVE' as const }]} selected="robot_2" mapId="mock_lab" onSelect={() => {}} />)
  expect(screen.getByText(/상태 확인 중/)).toBeTruthy()
  expect(document.querySelectorAll('.scan-layer circle').length).toBe(0)
