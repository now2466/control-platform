...
def _filters(request: Request, user: UserInfo, event_type: str | None, robot_id: str | None, mission_id: str | None, from_time: datetime | None, to_time: datetime | None, cursor: int | None, limit: int) -> tuple[list[dict[str, object]], int | None]:
    ...
    return request.app.state.storage.history(user, event_type=event_type, robot_id=robot_id, mission_id=mission_id, from_time=_timestamp(start), to_time=_timestamp(end), limit=limit, cursor=cursor)
...
async def export_history(request: Request, event_type: str | None = None, robot_id: str | None = None, mission_id: str | None = None, from_time: datetime | None = Query(None, alias="from"), to_time: datetime | None = Query(None, alias="to"), cursor: int | None = None, limit: int = 100, user: UserInfo = Depends(current_user)) -> Response:
    items, next_cursor = _filters(request, user, event_type, robot_id, mission_id, from_time, to_time, cursor, limit)
    return Response(content=json.dumps({"items": items, "next_cursor": next_cursor}, ensure_ascii=False), media_type="application/json", headers={"Content-Disposition": "attachment; filename=history.json", "Cache-Control": "no-store"})
